...
export function useGameSession() {
  ...
  return {
    ...
    error,
    setError,
    refresh: () => run("Refreshing", api.state),
    refreshScenarios: () =>
      api
        .scenarios()
        .then(setScenarios)
    ...
    toggleDebug: () => run("Toggling debug", () => api.toggleDebug(null)),
    ...
    freeformAction: (text: string) => run("Resolving action", () => api.freeformAction(text)),
    ...
    endTurn: () => run("Ending turn", api.endTurn),
